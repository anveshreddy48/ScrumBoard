"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
var core_1 = require("@angular/core");
var data_service_1 = require("../core/data.service");
var BacklogComponent = (function () {
    function BacklogComponent(dataService) {
        this.dataService = dataService;
    }
    BacklogComponent.prototype.ngOnInit = function () {
        this.getBacklogStories();
    };
    BacklogComponent.prototype.getBacklogStories = function () {
        var _this = this;
        this.dataService.getBacklogStories()
            .subscribe(function (response) {
            console.log('component response', response);
            _this.stories = response;
        }, function (err) { return console.log(err); }, function () { return console.log('Backlog stories are now retrieved'); });
    };
    BacklogComponent = __decorate([
        core_1.Component({
            moduleId: module.id,
            selector: 'backlog',
            templateUrl: 'backlog.component.html'
        }),
        __metadata("design:paramtypes", [data_service_1.DataService])
    ], BacklogComponent);
    return BacklogComponent;
}());
exports.BacklogComponent = BacklogComponent;
//# sourceMappingURL=backlog.component.js.map